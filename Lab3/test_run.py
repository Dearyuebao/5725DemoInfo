...
while code_run:
	...
	if (y > 120 and x > 160) :
	...
	elif (x < 120 and y > 120) or someFlag:
		print("Start Pressed")
		if( x < 120 and y > 120):
			if(switch < 7):
				switch = switch + 1
			...
			while time.time() - now < 1.5:
				for event in pygame.event.get():
					if event.type is MOUSEBUTTONDOWN:
						...
						pos = pygame.mouse.get_pos()
						x, y = pos
				if 120 < x < 160 and 100 < y < 140:
					# pLeft.stop()
					# pRight.stop()
					pRight.ChangeDutyCycle(0)
					#pRight.ChangeFrequency(0)
					pLeft.ChangeDutyCycle(0)
					#pLeft.ChangeFrequency(0)
					someFlag = False
					switch = 1
					screen.fill(BLACK)
					display("resume")
					break
				switch = 2
				# pRight.start(100*1.6/21.6)
				# pRight.ChangeFrequency(1000/21.6)
				pRight.ChangeDutyCycle(100*1.6/21.6)
				pRight.ChangeFrequency(1000/21.6)
				pLeft.ChangeDutyCycle(100*1.4/21.4)
				pLeft.ChangeFrequency(1000/21.4)
			if someFlag :
				leftq.pop(0)
				leftq.append(("Forward", str(int(time.time()-start))))
				rightq.pop(0)
				rightq.append(("Forward", str(int(time.time()-start))))
				screen.fill(BLACK)
				display("stop")
		...
		if(switch == 3):
			...
			while time.time() - now < 1.5:
				...
				pRight.ChangeDutyCycle(100*1.4/21.4)
				pRight.ChangeFrequency(1000/21.4)
				pLeft.ChangeDutyCycle(100*1.6/21.6)
				pLeft.ChangeFrequency(1000/21.6)
				switch = 4
			if someFlag :
				leftq.pop(0)
				leftq.append(("Back", str(int(time.time()-start))))
				rightq.pop(0)
				rightq.append(("Back", str(int(time.time()-start))))
				screen.fill(BLACK)
				display("stop")
